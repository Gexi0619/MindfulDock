...
function getLatestLocalVideoDate(localVideos) {
    if (!localVideos || localVideos.length === 0) {
        return '20240101'; // 默认时间：2024年1月1日
    }

    // 获取最晚的日期
    const latestDate = localVideos
        .map(video => video.upload_date)
        .filter(date => date) // 过滤无效日期
        .sort((a, b) => b.localeCompare(a))[0]; // 降序排列，取最晚日期

    return latestDate || '20240101';
}


// 获取远程所有视频信息，包括日期和时长
async function getRemoteVideos(source, globalSettings, latestLocalDate) {
    const options = [
        ...globalSettings['yt-dlp']?.options || [],
        ...source.custom_settings?.['yt-dlp']?.options || [],
        '--dateafter', latestLocalDate,
        '--flat-playlist', '--print', '%(id)s\t%(title)s'
    ];
    const command = ['yt-dlp', source.url, ...options];

    return new Promise((resolve, reject) => {
        console.log(`Fetching remote videos for: ${source.name} (after ${latestLocalDate})`);
...
async function main() {
    ...
    try {
        ...
        for (const source of sources) {
            ...
            // 获取该频道本地视频最晚的上传日期
            const latestLocalDate = getLatestLocalVideoDate(localVideos[source.name]);
            console.log(`Latest local video date for ${source.name}: ${latestLocalDate}`);

            // 获取远程需要下载的视频信息
            const remoteVideos = await getRemoteVideos(source, globalSettings, latestLocalDate);
